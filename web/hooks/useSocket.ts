...
export const useSocket = (namespace: string) => {
  ...
  const connectSocket = useCallback(async () => {
    ...
    try {
      ...
      newSocket.on('disconnect', (reason) => {
        ...
        setIsConnected(false);
        // If the disconnection was not initiated by the client, it will attempt to reconnect.
      });
      ...
      newSocket.io.on('reconnect_attempt', async (attempt) => {
        ...
        try {
          ...
          newSocket.auth = { token: res.data.socketToken };
        } catch (err) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (socketRef.current) {
        console.log(`Cleaning up socket connection from namespace: ${namespace}`);
        socketRef.current.disconnect();
        socketRef.current = null;
      }
    };
