    ...
    return (
        <div className="space-y-6">
            ...
            <Table>
                ...
                <TableBody>
                    {isLoading ? <TableRow><TableCell colSpan={6} className="text-center">Loading reports...</TableCell></TableRow>
                    : reports?.data.map((report: any) => (
                        <TableRow key={report._id}>
                            <TableCell className="font-medium">{report.title}</TableCell>
                            <TableCell>{report.isAnonymous ? "Anonymous" : report.owner.name}</TableCell>
