...
// Global sanitization for XSS protection
...
peer.use((socket, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
    if (err) {
      ...
      if (err.name === 'TokenExpiredError') errorMessage = "Token expired. Please refresh.";
...
peer.on("connection", async (socket) => {
  ...
  // =================================================================
  // START: UPDATED JOIN LOGIC
  // =================================================================
  socket.on("join", async ({ topic, sessionId }) => {
    try {
      ...
      let roomTopic, roomId, isModerator = false;

      // --- LOGIC FOR SCHEDULED SESSIONS ---
      if (sessionId) {
        const session = await ScheduledSession.findById(sessionId);
        if (!session || session.status !== 'scheduled') {
          return socket.emit("error", { message: "This session is not available." });
        }
        roomId = `session-${sessionId}`;
        roomTopic = session.topic;
        isModerator = session.volunteerModerator.toString() === socket.user.id;
        socket.isModerator = isModerator; // Attach moderator status to socket
      
      // --- EXISTING LOGIC FOR TOPIC ROOMS ---
      } else if (topic) {
        ...
        return socket.emit("error", { message: "Topic or Session ID is required to join." });
      }

      currentRoomId = roomId;
      socket.join(roomId);
      
      socket.emit("joined", { roomId, topic: roomTopic, isModerator });
  ...
  socket.on("message", async ({ roomId, text }) => {
    try {
      ...
      const msg = await Message.create({
        room: roomId, sender: socket.user.id, senderModel, content: cleanText
      });
      const messagePayload = {
        id: msg._id, roomId, text: msg.content,
  ...
  socket.on("typing", ({ roomId, typing }) => {
    if (roomId && typeof typing === 'boolean') {
      socket.to(roomId).emit("typing", {
        userId: socket.user.id, name: socket.user.name, typing: !!typing
      });
    }
  });
  ...
  socket.on("history", async ({ roomId, limit = 50, offset = 0 }) => {
    try {
      ...
      const msgs = await Message.find({ room: roomId })
        .sort({ createdAt: -1 }).skip(safeOffset).limit(safeLimit)
        .populate('sender', 'name role');
      socket.emit("history", {
        messages: msgs.reverse().map(m => ({
            id: m._id, text: m.content,
            sender: { id: m.sender?._id, name: m.sender?.name || m.senderModel, role: m.sender?.role || m.senderModel.toLowerCase() },
            createdAt: m.createdAt
        })),
        hasMore: msgs.length === safeLimit, offset: safeOffset + safeLimit
      });
    } catch (error) {
...
privateChat.use((socket, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
    ...
    if (!decoded.id || !decoded.role) { return next(new Error("Authentication error: Invalid token payload.")); }
...
privateChat.on("connection", (socket) => {
    ...
    socket.on("join", async ({ recipientId, recipientRole }) => {
        try {
            ...
            if (!recipientId || !recipientRole) { return socket.emit("error", { message: "Recipient ID and role required" }); }
            ...
            if (!validRoles.includes(recipientModel)) { return socket.emit("error", { message: "Invalid recipient role" }); }
            const modelsMap = { Student, Counsellor, Volunteer, Admin };
            const recipientExists = await modelsMap[recipientModel].exists({ _id: recipientId });
            if (!recipientExists) { return socket.emit("error", { message: "Recipient not found" }); }
            const roomId = [myId, recipientId].sort().join('-');
            socket.join(roomId);
            let conversation = await Conversation.findOne({ "participants.user": { $all: [myId, recipientId] } });
            if (!conversation) {
                const myUserModel = socket.user.role.charAt(0).toUpperCase() + socket.user.role.slice(1);
                conversation = await Conversation.create({
                    participants: [{ user: myId, userModel: myUserModel }, { user: recipientId, userModel: recipientModel }]
                });
            }
    ...
    socket.on("message", async ({ roomId, conversationId, text }) => {
        ...
        try {
            const cleanText = sanitizeHtml(text.slice(0, 2000), { allowedTags: [], allowedAttributes: {} });
            const senderModel = socket.user.role.charAt(0).toUpperCase() + socket.user.role.slice(1);
            const msg = await PrivateMessage.create({
                conversation: conversationId, sender: socket.user.id, senderModel, content: cleanText
            });
            await Conversation.findByIdAndUpdate(conversationId, { lastMessage: msg._id });
            const populatedMsg = await PrivateMessage.findById(msg._id).populate('sender', 'name role profileImage');
            ...
            await Notification.create({
                recipient: recipientParticipant.user._id, recipientModel: recipientParticipant.userModel,
                sender: socket.user.id, senderModel: senderModel, type: 'message_received', category: 'chat',
                title: `New Message from ${populatedMsg.sender.name}`, message: populatedMsg.content,
                data: { conversationId: conversation._id, senderId: socket.user.id, senderName: populatedMsg.sender.name, },
  ...
  cron.schedule('0 10 * * 0', async () => {
    ...
    try {
      ...
      const studentsToNotify = await Student.find({
        ...
        _id: { $nin: await StudentCheckin.distinct('student', { createdAt: { $gte: oneWeekAgo } }) }
      }).select('_id');
      const recipients = studentsToNotify.map(s => ({ userId: s._id, userModel: 'Student' }));
      if (recipients.length > 0) {
        await Notification.createSystemNotification(
          recipients, 'checkin_reminder', 'How are you feeling?',
