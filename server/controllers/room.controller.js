...
// FILE: server/controllers/room.controller.js
import Room from "../models/room.model.js";
import Message from "../models/message.model.js";
import { PEER_TOPICS } from "../constants/peer.js";


// Get all available rooms (includes default topics)
export const getAllRooms = async (req, res) => {
  try {
    ...
    const allRooms = [...rooms, ...defaultRooms].sort((a, b) => a.topic.localeCompare(b.topic));
...
// Get room by topic (handles both custom and default rooms)
export const getRoomByTopic = async (req, res) => {
  try {
    const { topic } = req.params;
    let room = await Room.findOne({ topic }).populate('moderators', 'name email');
    
    // If room doesn't exist but is a default topic, create it
    if (!room && PEER_TOPICS.includes(topic)) {
...
// Get room messages (handles both custom and default rooms)
